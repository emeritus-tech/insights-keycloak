...
CMD ["java", "-Dkc.home.dir=/app/quarkus/server/target", "-jar", "--enable-preview", "/app/quarkus/server/target/lib/quarkus-run.jar", "start", "--spi-migration-strategy", "update", "--spi-datastore-legacy-allow-migrate-existing-database-to-snapshot", "true", "--db", "mysql", "--db-username", "$DB_USERNAME", "--db-password", "$DB_PASSWORD", "--hostname", "$HOSTNAME", "--db-url-host", "$DB_HOST", "--proxy-headers", "xforwarded", "--db-url-database", "$DB_NAME", "--cache", "ispn", "--cache-stack", "kubernetes"]
