...
CMD ["java", "-jar", "/app/quarkus/server/target/lib/quarkus-run.jar", "start","--spi-migration-strategy", "update", "--db", "mysql", "--db-username", "$DB_USERNAME", "--db-password", "$DB_PASSWORD", "--hostname", "$HOSTNAME", "--db-url-host", "$DB_HOST", "--proxy", "edge", "--db-url-database", "$DB_NAME", "--cache", "ispn", "--cache-stack", "kubernetes"]
